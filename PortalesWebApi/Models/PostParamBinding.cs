...
{
    public class PostParamBinding : HttpParameterBinding
    {
        ...
        public PostParamBinding(HttpParameterDescriptor descriptor)
        : base(descriptor)
        { }
        ...
        /// <returns></returns>
        public static HttpParameterBinding PostParameterBinding(HttpParameterDescriptor descriptor)
        {
            //To see is it mark the flag
            if (descriptor.ActionDescriptor.GetCustomAttributes<PostParamAttribute>().Count <= 0)
            {
                return null;
            }
            ...
            if (supportedMethods.Contains(HttpMethod.Post) || supportedMethods.Contains(HttpMethod.Put))
            {
                ...
                {
                    return new PostParamBinding(descriptor);
                }
        ...
        object StringToType(string stringValue)
        {
            ...
            else if (Descriptor.ParameterType == typeof(bool?))
            {
                ...
                {
                    if (stringValue.Equals("true", StringComparison.OrdinalIgnoreCase) || stringValue.Equals("on", StringComparison.OrdinalIgnoreCase) || stringValue == "1")
                    {
                        value = true;
                    }
                }
